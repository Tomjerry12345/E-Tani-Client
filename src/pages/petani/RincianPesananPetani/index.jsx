...
export default function RincianPesanan() {
    ...
    const btnTerima = (id, message) => {
        ...
        axios.put(`http://localhost:4000/rincian-pesanan/update/${id}`, {
            data,
            headers: {
                "content-type": "multipart/form-data",
            },
        ...
        <TableContainer component={Paper}>
            <Table className={classes.table} aria-label="simple table">
                ...
                <TableBody>
                    {data.map((row, index) => (
                        <TableRow key={index}>
                            <TableCell component="th" scope="row">{row.namaProduk.map((res, index) => `${index + 1}. ${res}`)}</TableCell>
                            <TableCell align="right">{row.harga.map((res, index) => `${index + 1}. ${res}`)}</TableCell>
                            <TableCell align="right">{row.jumlah.map((res, index) => `${index + 1}. ${res}`)}</TableCell>
                            ...
                            <TableCell align="right">
                                <Button
                                    ...
                                    variant="contained" onClick={() => btnTerima(row._id, row.statusPengiriman)}>{row.statusPengiriman === "Belum Terkirim" ? "Belum Terkirim" : "Sudah Terkirim"}</Button>
                            </TableCell>
                        </TableRow>
                    ))}
                </TableBody>
